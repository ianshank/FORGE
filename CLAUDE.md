...
- `scripts/openspec validate --strict` — Validate OpenSpec change proposals and specifications strictly (CI: `openspec-validate` job)
- `docs/v0.6.0-operator-runbook.md` — Release operator runbook for Decision D2 default branch rename, Pages/HF_TOKEN setup, and `v0.6.0` annotated tag
...
- `python scripts/train.py --agent mappo --num-updates N --device {auto,cpu,cuda,cuda:N,mps}` — Local MAPPO training (needs `maturin develop --extras train`). `--device` > `FORGE_HARDWARE_DEVICE` > `[hardware] device` (`forge.toml` ships `auto`); an unusable explicit accelerator exits 1 before the env is built. README "Train an Agent Locally" covers the CUDA install + MuZero-on-GPU path
...
- `UPDATE_GOLDEN_REPLAYS=1 cargo test -p forge-replay --test golden_replay` — regenerate CompactReplay v2 goldens after `ForgeConfig` field additions; then append `docs/results/replay_flip_log.md`. Do not combine UPDATE with a snapshot-read in the same cargo invocation (tests run in parallel). Weekly / `workflow_dispatch` `.github/workflows/golden-replay.yml` runs `scripts/check_golden_replays.sh` (full crate suite). That script is **not** in `make verify`; workspace `cargo test` already runs `golden_replay`
