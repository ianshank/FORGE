...
:meth:`ForgeDataset.to_arrays` helper that returns plain NumPy arrays suitable
for offline RL methods (JAX / Flax / StableBaselines3).
...
# ---------------------------------------------------------------------------
# Optional dependency: minari (lazy import so the module loads without it)
# ---------------------------------------------------------------------------
try:
    import minari as _minari
except ImportError:
    _minari = None
...
# ---------------------------------------------------------------------------
# Observation shape constants (match ForgeGymnasiumEnv defaults)
# ---------------------------------------------------------------------------
...
class ForgeDataset:
    ...
    compatibility, and provides :meth:`to_arrays` for bulk NumPy export.
    """
    ...
    # ------------------------------------------------------------------
    # Sequence protocol (PyTorch Dataset interface)
    # ------------------------------------------------------------------
    ...
    def to_arrays(
        ...
        observations : np.ndarray, shape (N, obs_dim)
            Flattened, normalised observation vector
            (grid_view + inventory + scalar fields).
        ...
        # Determine obs dimension from the first step.
        obs_dim = self._flatten_obs(self._steps[0]).shape[0]

        obs_arr: np.ndarray = np.empty((n, obs_dim), dtype=np.float32)
        act_arr: np.ndarray = np.empty(n, dtype=np.int32)
        rew_arr: np.ndarray = np.empty(n, dtype=np.float32)
        done_arr: np.ndarray = np.empty(n, dtype=bool)
...
# ---------------------------------------------------------------------------
# Shared JSONL step parser (forge-replay format)
# ---------------------------------------------------------------------------


def _parse_forge_jsonl_step(raw: dict[str, Any], tick: int) -> ForgeStep | None:
    ...
    try:
        ...
        vw: int = obs.get("view_width", _DEFAULT_VIEW_SIZE)
        vh: int = obs.get("view_height", _DEFAULT_VIEW_SIZE)
        ...
        pos_raw: list[int] = obs.get("position", [0, 0])
...
def load_forge_jsonl(path: str, max_steps: int = 0) -> ForgeDataset:
    ...
    path : str
        ...
        ``forge_replay::export::export_to_jsonl`` or
        :meth:`forge_data.loader.OfflineDataset.export_jsonl`.
    max_steps : int
    ...
    with Path(path).open() as fh:
        for tick, raw_line in enumerate(fh):
...
def load_minari(
    ...
    dataset = _minari.load_dataset(dataset_id, download=True)

    steps: list[ForgeStep] = []

    for ep_count, episode in enumerate(dataset.iterate_episodes(), start=1):
        ...
        if (max_episodes > 0 and ep_count >= max_episodes) or (
            max_steps > 0 and len(steps) >= max_steps
        ):
            break
...
# Craft target name fragments → FORGE recipe index.
# Longer/more-specific patterns must precede shorter ones (e.g. "pickaxe" before
# "axe") to avoid substring false-matches.
_MINERL_CRAFT_MAP: tuple[tuple[str, int], ...] = (
    ("pickaxe", 1),  # before "axe"
    ("axe", 0),
    ("plank", 2),
    ("bridge", 3),
    ("sword", 4),
...
_FORGE_CRAFT_BASE: int = 26


def _minerl_action_to_discrete(action: dict[str, Any]) -> int:
    """Convert a MineRL action dict to a FORGE discrete action id.

    Priority: craft > boolean actions > noop.
    """
    if action.get("no_op"):
        return 0
    ...
    if craft:
        craft_lower = str(craft).lower()
        for fragment, recipe_idx in _MINERL_CRAFT_MAP:
            if fragment in craft_lower:
                return _FORGE_CRAFT_BASE + recipe_idx
        return 0  # unknown craft -> noop

    for key, action_id in _MINERL_BOOL_ACTIONS:
        if action.get(key):
            return action_id
...
def load_minerl(
    ...
    with Path(path).open() as fh:
        for raw_line in fh:
            ...
            action_id = _minerl_action_to_discrete(raw.get("action", {}))
...
# Direction character → FORGE discrete Move action id (comm_vocab=0, no drone).
_MAZE_DIR_TO_ACTION: dict[str, int] = {"U": 1, "D": 2, "L": 3, "R": 4}

# Intermediate reward per non-terminal maze step.
_MAZE_STEP_REWARD: float = 0.01
# Terminal reward on reaching the goal.
_MAZE_GOAL_REWARD: float = 1.0
...
def load_maze_jsonl(
    ...
    where every BFS move is one ``Move`` action.  The final step carries a
    reward of :data:`_MAZE_GOAL_REWARD`; all others carry
    :data:`_MAZE_STEP_REWARD`.
