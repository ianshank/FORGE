...
| `docker compose -f docker/compose.minecraft.yml --env-file docker/compose.minecraft.env up -d minecraft mc-bot runner` | **v0.5 Phase 1** — full v0.5 stack bring-up. Compose mounts `configs/minecraft/env.docker.toml` over `env.toml` so the bot uses docker DNS hostnames (`minecraft`, `mc-bot:8766`) instead of local-dev `127.0.0.1` defaults |
| `python scripts/v05_handshake_probe.py 127.0.0.1 8766` | **v0.5 Phase 1** — stdlib-only WS handshake probe. Connects to live mc-bot, validates v0.5 contract end-to-end (`obs_dim=920`, `grid_shape={11,11,1,7,73}`). EXIT_OK / EXIT_GRID_SHAPE_MISSING / EXIT_GRID_SHAPE_MISMATCH for CI gating |
