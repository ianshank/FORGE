...
| `docker build -f docker/mc-runner.Dockerfile -t forge-mc-runner:dev .` | **v0.5 Phase 1** — runner image (rust:1.93-bookworm builder, 135 MB debian:bookworm-slim runtime). Builds `--features mc-live` (random-only) or `--features mc-live-bundled` (trained-mode; fixed in the 2026-08 tech-debt pass -- 5-line `ort` rc.13 API-drift fix in `onnx_model.rs` plus bumping `ONNXRUNTIME_VERSION` to 1.23.2, which resolves a known upstream teardown segfault under `load-dynamic`, pykeio/ort#614) |
