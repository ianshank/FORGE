...
- **Runner continue-on-reconnect**: mid-episode `RECONNECTING`/`BUSY` discards the partial trajectory (`discard_current`), records `METRIC_REASON_ENV_STEP` or `METRIC_REASON_ENV_RESET` protocol errors, and continues `Runner::run` after a config-driven backoff (`max_consecutive_transient_failures` default 3, `transient_failure_backoff_ms` default 500). `parse_transient_env_error` accepts only those two codes (unknown/`INTERNAL` fail closed). Do not retry `recv` or resend Step -- reconnect is a new MDP. Typed `McEnvError::Transient` at the protocol boundary; `RunnerError::TransientEnv` / `TooManyTransientFailures` on the generic runner.
...
- **Nested rewards in schema_id**: `config_path` / `crafting_config_path` resolve relative to `parent(rewards.toml)` and the nested file's canonical SHA replaces the path string in the hash tree. Rename-without-content-change does not bump. Missing nested files fail closed. Fixture xlang pin unchanged; new shipped-config pin across Rust/JS/Python. Block embeddings remain a separate obs-layout pin; `MuZeroConfig.num_block_embeddings` loads `max(index)+1` from the TOML vocab.
- **Evidential measurement**: live Paper/MC capture is still ops-blocked (no Docker in this environment). Runner reconnect survival is pinned by MockBot integration tests. `docs/results/v0.5-loop-survival.md` records remaining operator steps. `v0.5-trained-vs-random.md` stays a template until both live variants clear the evidential floor of 3.
- **Loop-survival hygiene**: pinned Display-prefix twins (`TRANSIENT_PROTOCOL_ERROR_DISPLAY_PREFIX` / `TRANSIENT_ENV_DISPLAY_PREFIX`), Hello.schema_id two-input contract (embeddings stay a separate obs-layout pin), RECONNECTING/BUSY wire-string pins, nested-path negatives (empty / non-string / crafting_config_path / sibling isolation for docker `/app/configs`), BUSY continue-on-transient, embeddings missing-file warning, advisory `guard_schema_id_pins.py` PreToolUse hook, C4/README/Agent/config-catalog updates. `.gitignore` exception for `python/forge/models/*.py`. Dependabot toml 1.x / tungstenite 0.30 / python 3.14 Docker remain parked.
- **CI / review triage**: `parse_transient_env_error` rejects non-`RECONNECTING`/`BUSY` codes even when they use the transient Display prefix; reset vs step transients record distinct Prometheus `reason` labels; mc-bot `canonicalSha256()` is memoized at load (handshake hash); unused `# noqa: PLC0415` removed (`PLC0415` is globally ignored); `v0.5-trained-vs-random.md` MD028 blank-in-blockquote fixed without filling evidential placeholders.
...
- **God File Elimination & Modularization**:
  ...
  - Extracted in-file tests from `crates/forge-eval/src/harness.rs`, `crates/forge-eval/src/exporters/mlflow_http.rs`, `crates/forge-mc-runner/src/runner.rs`, `crates/forge-task/src/predicate.rs`, and `crates/forge-cloud/src/gcs_storage.rs` via the same `#[path]` pattern (production files no longer host 300–770 line test modules). All identified production god files are now decomposed.
