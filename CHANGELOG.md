...
#### Kitchen-Counter Cleanup Robot Deployment (PR #41)

- Added **`forge-actuator`** crate (`crates/forge-actuator/`) — a generic, config-driven action-id → actuator-command bridge for FORGE edge deployments. Public surface: `ActuatorCommand` (`drive_direction`, `open_gripper`, `close_gripper`, `engage_sweeper`, `disengage_sweeper`, `vibrate`, `halt`, `custom`), `CardinalDirection`, `ActuatorBridge` + `ActuatorDriver` traits, `MappedActuator<D>` (single generic implementor wired through `tracing` with a bounded command-history ring buffer), `ActionMapping` (TOML-driven with `strict` / permissive modes, duplicate-id rejection, missing-default rejection, single-probe `resolve()` for the dispatch hot path), `MockDriver` for tests with optional failure-injection predicate, `ActuatorError` (`#[non_exhaustive]`, with `UnknownActionId`, `DriverFailure`, `InvalidMapping`, `ParseToml`, `Io` variants). 37 unit tests including proptest cases.
- Added **`EdgeConfig.fallback_action_id`** field (default `0` / Noop) backed by `DEFAULT_EDGE_FALLBACK_ACTION_ID` in `forge-types::constants`, wired through `apply_env_overrides` (`FORGE_EDGE_FALLBACK_ACTION_ID`). `EdgeAgent::new` now reads this field instead of the previous hardcoded `0`, and a new `EdgeAgent::fallback_action_id()` accessor exposes it. Backwards-compatible: existing TOMLs without the field deserialize to `0` so behaviour is unchanged for legacy edge profiles.
- Added **three deployment configs**: `configs/edge/rpi5_hailo_kitchen.toml` (kitchen `EdgeConfig` profile — 30 Hz budget, 4 MiB telemetry buffer, `fallback_action_id = 17`), `configs/actuator/openclaw_kitchen.toml` (concrete OpenClaw action-id mapping including the safe-pose `17 → [disengage_sweeper, halt]` contract), `configs/scenarios/kitchen_cleanup.toml` (a real `forge_scenario::ScenarioConfig` — verified to parse via the existing loader by a regression test).
- Added **`tests/rust/integration_kitchen_robot.rs`** — 10 workspace integration tests exercising config loading, the cleanup-sweep dispatch sequence, the end-to-end safe-pose fallback path (always-failing model → `EdgeAgent` emits `fallback_action_id = 17` → bridge dispatches `[disengage_sweeper, halt]` to a `MockDriver`), the legacy-profile backwards-compat path, and a `ScenarioConfig::from_toml` parse for the kitchen scenario.
- Added **`docs/kitchen_counter_robot.md`** — RPi 5 + Hailo AI HAT 2 + OpenClaw deployment proposal that grounds every claim in actual file paths and proves the config-only delta with the integration test above.
