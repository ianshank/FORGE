...
#### Bincode 2.x Wire-Format Migration Roadmap

FORGE currently uses `bincode 1.3.3` for serializing `WorldState` snapshots, checkpoints, and golden hash determinism tests (`crates/forge-core/src/world/serialize.rs`). Upstream development on `bincode 1.x` has ceased (tracked as reviewed exception `RUSTSEC-2025-0141` in `deny.toml`). Because bincode 2.x introduces breaking API changes and an altered binary encoding format, migration follows this strict four-phase invariant-preserving roadmap:

1. **Golden Binary Snapshot Corpus**: Stage a comprehensive suite of binary test snapshots generated by bincode 1.3.3 across multiple canonical seeds and world configurations to serve as the ground-truth regression gate.
2. **Dual-Engine Implementation**: Implement the bincode 2.x serializer behind a dedicated Cargo feature flag (`bincode-v2`) in `crates/forge-core` and `crates/forge-types`.
3. **Format Migration Adapter (`v1_to_v2`)**: Provide an explicit binary migration converter that automatically detects legacy v1 headers, deserializes older state representations, and writes the upgraded v2 encoding.
4. **Deprecation & Advisory Retirement**: Once all downstream consumers and golden determinism tests validate cleanly against v2, deprecate the bincode 1.x codepaths and remove exception `RUSTSEC-2025-0141` from `deny.toml`.
...
FORGE establishes a clear, tiered stability policy across its architectural layers:

### Tiered Stability Guarantees

1. **Foundation & Core Engine (`[Production]`)**:
   - Covers Tier 0 (`forge-types`, `forge-env`, `forge-observability`), Tier 2 (`forge-core`), and core runtime crates (`forge-agent`, `forge-server`, `forge-python`, `forge-wasm`).
   - Strict Semantic Versioning: Public APIs, types (`ForgeConfig`, `Action`, `Position`, `StepResult`), and zero-allocation hot-path signatures (`WorldState::step_into`) will not undergo breaking changes without a major/minor version bump and formal migration notice.
   - Any modification must maintain bit-identical determinism (Invariant 6) and zero-allocation contracts (Invariant 4).

2. **Cross-Language Wire Protocols & Schemas (`[Production]`)**:
   - Covers TrajectoryV2 (`forge-replay`), WebSocket protocols (`forge-env-mc`, `mc-bot`), and runner manifests (`forge-mc-runner`).
   - Forward and Backward Compatibility: All wire format evolution must be additive. New schema versions require backward-compatible readers or explicit format migration converters (Invariant 2).
   - Cross-language consistency is defended by synchronized hash pinning across Rust, Node, and Python.

3. **Research Stack & Experimental Subsystems (`[Research]`, `[Experimental]`)**:
   - Covers research primitives (`forge-memory`, `forge-social`, `forge-cognitive`, `forge-edge`, `forge-mangomas`, `forge-proposal`, `forge-cloud`).
   - Designed for high-velocity research and experimentation. Public APIs in these modules may iterate across minor releases, provided that all workspace integration tests in `tests/rust/` remain fully functional and breaking changes are documented in release notes.
