...
| Remaining production god files (`mlflow_http.rs`, `harness.rs`, `runner.rs`, `predicate.rs`) | Resolved | **[STATUS: LANDED 2026-09]** Extracted in-file tests from `harness.rs`, `mlflow_http.rs`, `runner.rs`, and `predicate.rs` using `#[path = ".../tests.rs"]`. `gcs_storage.rs` still mixes `#[cfg(test)]` helpers into production and is deferred. Do not split hot-path bodies (`WorldState::step_into`) without an allocation-audit follow-up. |
...
| Baseline aggregates include non-episodes | High | `scripts/mc_plot_baseline.py::summarize_snapshot` averages `total_reward` over every record with no evidential filter, and `scripts/v05_manual_baseline.py` marks an environment-reported step failure as `truncated` and continues. The committed `docs/results/v0.5-first-real-run-baseline*.json` therefore carry 29/30 and 9/10 records whose final step attempt returned an error frame rather than an observation, indistinguishable from a budget truncation by the `terminated`/`truncated` fields alone. Tracked as [`openspec/changes/refuse-non-evidential-aggregates/`](../openspec/changes/refuse-non-evidential-aggregates/). |
