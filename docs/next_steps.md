...
(a default-off `openapi` feature is the planned shape).

---

## Graded agri/drone loop (A0–F) — LANDED

Expand only where a grader can execute. Branch
`cursor/graded-loop-expansion-9c4d` (PR #161).

| Slice | Status | Where |
|---|---|---|
| A0 compiler | LANDED | `crates/forge-types/src/scenario.rs` + `python/forge/mangomas/collector/scenario.py`; xlang pin `xlang_*_pinned_to_known_good` / `tests/python/test_scenario_compiler_xlang_pin.py` |
| A1 predicates + eval success | LANDED | drone/agri predicates; `forge-eval` success = alive ∧ all attached tasks complete ∧ not truncated |
| B CompactReplay v2 | LANDED | portable SHA-256 `config_hash`, seed pin, `tests/golden/replays/v2_seed42.json`; fail-closed empty hash |
| C VecEnv SPS@N | LANDED | `benchmarks/baselines/cloud_agent/vecenv_step.json`; not JAX `vmap`; heavy sweep gated |
| D evidential capture | LANDED (script) | `scripts/mc_evidential_capture.sh` (`--dry-run` is CI). Live `evidential_episodes >= 3` is **not** produced in this environment |
| E orchard + charger/geofence | LANDED | `configs/scenarios/orchard_coverage.toml`; config-driven Noops |
| F OpenEnv sidecar | LANDED | `python/forge_env/openenv_env.py`; **not** on the PyO3 hot path |

Claude skill: `.claude/skills/forge-scenario-compiler/SKILL.md`. Advisory hook:
`.claude/hooks/guard_golden_replay.py`.
...
- **forge-core HRL skills vs Minecraft action_map**  `[STATUS: do-not-alias]`
  `configs/agents/skills_default.toml` is the engine options/HRL catalog.
  Do **not** alias Minecraft `action_map.toml` ids onto forge-core
  `Action` / skill ids. Remaining live ops:
  [`docs/results/v0.5-loop-survival.md`](results/v0.5-loop-survival.md).
...
| Item | Priority | Notes |
|---|---|---|
| Cargo machete unused dependency false positives | Resolved | **[STATUS: LANDED 2026-09]** Pinned false-positive derive-only dependencies in `[package.metadata.cargo-machete]` across 21 crates and promoted check to blocking in CI. |
| demo_ui report-only test coverage | Resolved | **[STATUS: LANDED 2026-09]** Gated `demo_ui/backend` test coverage at 70% threshold via `demo_ui/pytest.ini` (`--cov-fail-under=70`). Baseline measured at 90.38%. |
| Python ABI mismatch in Dockerfile | Resolved | **[STATUS: LANDED 2026-09]** Aligned Stage 3 runtime to Python 3.11 in `docker/Dockerfile`, matching `rust:1.94.1-bookworm` wheel-builder. |
| Silent config drift from missing serde attributes | Resolved | **[STATUS: LANDED 2026-09]** Added `#[serde(deny_unknown_fields)]` across `ForgeConfig` and 11 child structs in `crates/forge-types/src/config.rs`. |
| Rust in-file test bloat & god files | Resolved | **[STATUS: LANDED 2026-09]** Extracted 3,900+ lines of tests from `world.rs`, `physics.rs`, `config.rs`, `action.rs`. Modularized `forge-core::world`, `collector.py`, and `mc-bot/src/index.ts`. |
...
| OpenSpec strict validation & release baseline hardening | ✅ Done | **[STATUS: LANDED 2026-09]** Added `scripts/openspec` wrapper and `openspec-validate` CI/local gate running `openspec validate --all --strict`, paired with unit tests in `tests/python/test_openspec_validation.py`. Documented support matrix reality (Python 3.11 CI execution vs `requires-python >= 3.9`), fenced `cibuildwheel` and automated PyPI publishing as deferred follow-ons, enforced regression gates, and authored operator release runbook `docs/v0.6.0-operator-runbook.md`. Change package `openspec/changes/v0.6.0-release-baseline-hardening/`. |
...
| `demo_ui` as installable package | ✅ Done | **[STATUS: LANDED 2026-09]** Fixed `[tool.setuptools]` so editable install maps `demo_ui = "."` with explicit packages `demo_ui`, `demo_ui.backend`. Verified `import demo_ui.backend` with empty `PYTHONPATH`. |
| Section-level output parsing | Low | Parse structured data from `forge_demo.py` for richer stats |
| `run_demo.ps1` → `run_demo.sh` cross-platform | ✅ Done | **[STATUS: LANDED 2026-09]** `demo_ui/run_demo.sh` is the Bash launcher (port / `--no-browser` / `--skip-install`). README documents both launchers. |
...
| `torch.jit.trace` deprecation in MuZero export | ✅ Done | **[STATUS: LANDED 2026-09]** Secondary `export_torchscript()` path now uses `torch.export.export` / `torch.export.save` with `.pt2` ExportedProgram artefacts. `_validate_torchscript` and `tests/python/test_muzero_export.py` load via `torch.export.load`. Primary ONNX path and the Rust runner are unchanged. |
...
| No repo-checked-in Claude Code skills/hooks despite a large, repetitive validation surface | ✅ Done | Added `.claude/skills/forge-verify/SKILL.md`, docs-audit, PR-review, skills-catalog, **scenario-compiler**, and PreToolUse hooks in `.claude/settings.json`: `guard_tracked_deletion.py`, `guard_staged_secrets.py`, `guard_line_ending_drift.py`, advisory `guard_schema_id_pins.py` (Minecraft xlang-pin TOML), and advisory `guard_golden_replay.py` (scenario TOML / ForgeConfig fingerprint → CompactReplay goldens). `make hooks-test` discovers `test_*.py`. |
...
| Docker image Python ABI mismatch | ✅ Done (2026-09-06) | `docker/Dockerfile` wheel-builder stage uses `rust:1.94.1-bookworm` (Python 3.11), but runtime stage used `python:3.14-slim-bookworm`. Maturin embeds the Python ABI tag into the wheel filename (cp311), causing `pip install` to fail with "not a supported wheel on this platform". Fixed by changing runtime to `python:3.11-slim-bookworm`. Also removed unused `FORGE_CONFIG_PATH` env var since `forge-server` uses `ForgeConfig::default()` and never loads from TOML. |
| Rust `ForgeConfig` silently ignores unknown TOML fields | ✅ Done (2026-09-06) | Root `forge.toml` uses Python schema (`[hardware]`, `[simulation]`, `[training]`) which doesn't match Rust `ForgeConfig` schema (`[world]`, `[physics]`, `[agents]`). Without `deny_unknown_fields`, serde silently ignored all values. Added `#[serde(deny_unknown_fields)]` to `ForgeConfig` and all nested config structs, plus documentation clarifying the schema difference between Python and Rust configs. |
| `pip-audit` security scanner is advisory-only | ✅ Done (2026-09-06) | Promoted `pip-audit` from advisory (`\|\| true`) to blocking in `security.yml`. Verified clean on 2026-09-05 with "No known vulnerabilities found", exit 0. Updated header comment to reflect that `pip-audit` is now blocking alongside `cargo-deny` and `gitleaks`. |
| Stale `#[allow(dead_code)]` in `mlflow_http.rs` | ✅ Done (2026-09-06) | The `http` field in `MlflowHttpClient` had a stale `#[allow(dead_code)]` annotation left over from skeleton phase. Field is actually used in 7 places when `http-mlflow` feature is enabled. Removed the annotation. |
| Unused `listBuiltins()` export in mc-bot | ✅ Done (2026-09-06) | `mc-bot/src/reward/index.ts` exported `listBuiltins()` but it was never imported anywhere. Removed the unused export and added a comment noting it can be re-added if CLI/diagnostics need it. |
