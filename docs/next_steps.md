...
| CI Python installs use ad hoc `pip install "pkg>=x,<y"` ranges instead of syncing the committed `uv.lock` | Medium | `ci.yml`'s `python-lint`/`python-test`/`python-test-fast`/`demo-ui` jobs each hand-list version ranges rather than running `uv sync`/`uv pip sync` against `uv.lock` (1.3MB, committed at repo root) — so the lockfile isn't actually what CI installs from, a latent reproducibility gap. Switching needs its own validation pass (confirming the lockfile covers every package each job currently ad-hoc-installs — torch, onnx, onnxruntime, onnxscript, stable-baselines3, maturin, etc. — and could surface its own resolution conflicts), so it's tracked here rather than bundled into the CI-pinning pass that added `rust-toolchain.toml` and pinned the floating `wasm-pack`/`cargo-machete`/`cargo-deny` tool-install versions. |
...
| Rust toolchain (18 lines/8 files) and ONNX Runtime (2 files) version pins can drift silently — no single source of truth possible across TOML/YAML/Dockerfile | ✅ Done | `scripts/check_version_consistency.py` re-derives every copy (15 `dtolnay/rust-toolchain@stable toolchain:` inputs across 5 workflow files + 2 Dockerfiles' `RUST_IMAGE_TAG` against `rust-toolchain.toml`'s `channel`; `ci.yml`'s `onnx-features` job `ORT_VERSION` against `docker/mc-runner.Dockerfile`'s `ONNXRUNTIME_VERSION`) and fails on drift instead of eliminating the duplication outright. Verified against deliberately-introduced mismatches in both checks. Deliberately excludes `docker/trainer.Dockerfile`'s independent Python-wheel `ONNXRUNTIME_VERSION` (different artifact, legitimately different version). Wired into `make verify` and CI's `python-lint` job; see `docs/hardcoded-values-audit.md`. |
