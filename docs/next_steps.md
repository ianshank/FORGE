...
| Real LM Studio integration smoke test | Partial — Phase 1 done | Phase 1: pytest `lmstudio` marker + opt-in `python-test-lmstudio` CI job (workflow_dispatch) covering both supported model ids. Phase 2 (follow-up): full 1-episode `hex_patrol` collection via `collect_training_data_from_scenarios` — requires `base_forge_config` wiring. Pre-condition for the CI job: repo variable `LMSTUDIO_SERVICE_IMAGE` set to a tested OCI image. |
