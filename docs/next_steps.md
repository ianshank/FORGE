...
| Real LM Studio integration smoke test | Partial — Phase 1 done | Phase 1: `lmstudio` pytest marker registered (`pyproject.toml`), gates default suite via `addopts = "-m 'not lmstudio'"`. `tests/python/test_lmstudio_live_smoke.py` carries the marker plus a `FORGE_LMSTUDIO_LIVE=1` runtime gate. New `test_lmstudio_round_trip_parametrised_by_model_id` reuses the `lmstudio_model_id` fixture so both `qwen2.5-14b-instruct` and `google/gemma-4-e4b` are covered. CI: new `python-test-lmstudio` job (workflow_dispatch only, gated by `inputs.run_lmstudio`) spins up `${{ vars.LMSTUDIO_SERVICE_IMAGE }}` as a service container on port `LMSTUDIO_PORT`, with `docker logs` dump on failure. Phase 2 (follow-up): full 1-episode hex_patrol collection via `collect_training_data_from_scenarios` — needs `base_forge_config` wiring; deferred to keep this PR focused. Pre-condition: set repo variable `LMSTUDIO_SERVICE_IMAGE` to a tested OCI image exposing OpenAI v1 API on port 1234. |
