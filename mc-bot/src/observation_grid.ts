...
function blockPos(x: number, y: number, z: number): BlockPos {
  return { x, y, z };
}
...
function positiveIntOr(value: any, fallback: number): number {
  return Number.isInteger(value) && value > 0 ? value : fallback;
}

function nonNegIntOr(value: any, fallback: number): number {
  return Number.isInteger(value) && value >= 0 ? value : fallback;
}
 ...
 */
export function gridShape(config: any = {}): GridShape {
...
function blockLightLevel(block: any): number {
...
function blockHardness(block: any, hardnessScale: number): number {
...
function blockIsSolid(block: any): number {
  if (!block) return 0;
...
function blockIsLiquid(block: any): number {
  if (!block) return 0;
...
function blockIsDangerous(block: any, dangerousSet: Set<string>): number {
  if (!block) return 0;
  const name = blockTypeName(block);
  if (!name) return 0;
...
function biomeIdHash(block: any, biomeHashMod: number): number {
  if (!block) return 0;
...
function safeGetBlock(world: any, pos: BlockPos, missedTilesRef: { count: number }): any {
...
function blockTypeEmbeddingIndex(block: any, name: string, config: any): number {
  const useRaw = config.use_raw_block_id === true;
  const hashMod = config.block_id_hash_mod || DEFAULT_BLOCK_ID_HASH_MOD;
  if (!useRaw) {
    return stableStringHash(name, hashMod) / hashMod;
  }
  ...
  if (name && mappings[name] !== undefined) {
    return mappings[name];
  }
  if (mappings.unknown !== undefined) {
...
export interface EncodedBlockGrid {
  floats: number[];
  missedTiles: number;
  topBlockTypes: Array<[string, number]>;
  shape: GridShape;
}
 ...
 * health per episode.
 */
export function encodeBlockGrid(bot: any, config: any = {}): EncodedBlockGrid {
  const shape = gridShape(config);
  ...
  const blockHistogram = new Map<string, number>();
    ...
    for (let dx = -radius; dx <= radius; dx += 1) {
      for (let dz = -radius; dz <= radius; dz += 1) {
        ...
        const features = block
          ? [
              blockTypeEmbeddingIndex(block, name, config),
...
export interface GridShapePayload {
  height: number;
  width: number;
  depth: number;
  channels: number;
 ...
 */
export function gridShapePayload(config: any = {}): GridShapePayload | null {
