...
function validateGridShape(gridShape: Record<string, any>): GridShape {
 ...
 */
export function helloMsg({ actionCount, obsDim, schemaId, gridShape = null }: HelloArgs): HelloMessage {
 ...
 */
export function observationMsg({ tick, obs, reward, terminated, truncated, info }: ObservationArgs): ObservationMessage {
 ...
 */
export function errorMsg(code: string | number, message: string): ErrorMessage {
  return { type: 'error', code: String(code), message: String(message) };
}

export type ClientMessage =
  | { type: 'reset'; seed: number | null }
  | { type: 'step'; action_id: number }
  | { type: 'close' };
 ...
 */
export function parseClientMsg(input: string | Record<string, any>): ClientMessage {
